...
count_edge_stable( int color,
	...
	if ( color == CHESS_BLACK ) {
		edge_a1h1 = 3280 * CHESS_EMPTY - ix_a1h1;
		edge_a8h8 = 3280 * CHESS_EMPTY - ix_a8h8;
		edge_a1a8 = 3280 * CHESS_EMPTY - ix_a1a8;
		edge_h1h8 = 3280 * CHESS_EMPTY - ix_h1h8;
...
count_stable( int color,
	...
	if ( col_stable.high | col_stable.low )
		return non_iterative_popcount3( col_stable.high, col_stable.low );
	else
		return 0;
}
...
stability_search( BitBoard my_bits,
	...
	for ( old_sq = END_MOVE_LIST_HEAD, sq = stab_move_list[old_sq].succ;
		...
		if ( TestFlips_bitboard[sq]( my_bits, opp_bits ) ) {
			...
			BITBOARD_ANDNOT( bb_flips, my_bits );
